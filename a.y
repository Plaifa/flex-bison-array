...
line: assignment ';'		  	{ }
	...
	;
assignment: identifier '=' exp  { valUpdate($1,$3,NULL);  }
    | identifier ArLeft number ArRight '=' exp { valUpdate($1,$6,$3);  }
	;
exp: term                {$$ = $1;}
	| exp '+' exp          {$$ = $1 + $3;}
	| exp '-' exp          {$$ = $1 - $3;}
    ...
    ;
term: number              {$$ = $1;}
	| identifier						{$$ = val_of_iden($1,-1);} 
    | identifier ArLeft number ArRight  {$$ = val_of_iden($1,$3);} 
	;
%%                   
int val_of_iden(char * str,int idx){
	for(int i = 0; i < 255 ; i++){
		if(strcmp(sym[i].name ,str) == 0 && idx==-1)
        {
            return sym[i].value;
        }
        else if(strcmp(sym[i].name ,str) == 0 && idx==sym[i].idx)
        {
...
void valUpdate(char *str,int val,int idx)
{
    int i;
    i=var_counter;
    sym[var_counter].name = str;
    sym[var_counter].value = val;
    while (i>=0)
    {
            if (strcmp(sym[i].name,sym[var_counter].name)==0 && idx==NULL)
             {
                var_counter=i;
                sym[var_counter].value = val;
             }
             else if (strcmp(sym[i].name,sym[var_counter].name)==0  && idx==sym[i].idx)
             {
                sym[var_counter].value = val;
                sym[var_counter].idx = idx;
             }
        i--;
    }
    var_counter++;
